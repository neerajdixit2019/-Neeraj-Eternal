  ...
  return (
    <div className="mx-auto max-w-2xl px-5 py-10 sm:px-8 sm:py-14 space-y-10">
      <header className="fade-in">
        <p className="qs-section-label">{tx(lang, "your space, your way")}</p>
        <h1 className="mt-3 font-serif font-light tracking-tight text-3xl sm:text-[2.4rem] leading-tight">{tx(lang, "Sanctuary")}</h1>
        ...
        >
          ...
          {tx(lang, "in a hard moment? the steady room is one tap away — or call Tele-MANAS 14416.")}
        </Link>
      </header>
      ...
      <nav
        aria-label={tx(lang, "Sanctuary chapters")}
      ...
      <SectionGroup id="feel" label={tx(lang, "how it feels here")}>
        <LanguageSection />
        <ReadingSection />
      ...
      <SectionGroup id="letters" label={tx(lang, "the moon cycle")}>
        <SundayLetterSection />
      </SectionGroup>

      <SectionGroup id="inner-map" label={tx(lang, "the inner map — what InnerMate remembers")}>
        <InnerMapSection />
      </SectionGroup>

      <SectionGroup id="insight-sources" label={tx(lang, "personal insights — what your sky may learn from")}>
        <InsightSourcesSection />
      </SectionGroup>

      <SectionGroup id="vault" label={tx(lang, "your vault, your rules")}>
        <VaultSection />
        <LatchSection />
      </SectionGroup>

      <SectionGroup id="account" label={tx(lang, "account")}>
        <PasswordSection />
        <SignOutCard />
      </SectionGroup>

      <SectionGroup id="safety" label={tx(lang, "safety")}>
        <SafetySection />
      </SectionGroup>
      ...
      <footer className="pt-2 pb-4">
        <p className="flex items-center justify-center gap-2 text-center font-serif text-sm italic text-muted-foreground">
          ...
          {tx(lang, "everything you write and keep stays yours alone — never shared, never sold.")}
        </p>
      </footer>
    </div>
  );
...
function InsightSourcesSection() {
  ...
  return (
    <TactileCard tint="lavender">
      <h2 className="font-serif text-xl">{tx(lang, "What may your sky learn from?")}</h2>
      <p className="mt-1.5 text-sm leading-relaxed text-muted-foreground">
        {tx(lang, "Your Insights constellation is built only from sources you allow here. Turning one off removes its evidence immediately — nothing derived is ever stored, so there's nothing left behind to delete.")}
      </p>
          ...
          return (
            <div key={s.key} className="flex items-start justify-between gap-4 rounded-xl border border-border/50 bg-muted/20 p-4">
              <div className="min-w-0">
                <p className="text-sm font-medium">{tx(lang, s.title)}</p>
                <p className="mt-1 text-xs leading-relaxed text-muted-foreground">{tx(lang, s.line)}</p>
              </div>
              <button
                role="switch" aria-checked={on} aria-label={`${tx(lang, "Use")} ${tx(lang, s.title)} ${tx(lang, "for personal insights")}`}
      ...
      <p className="mt-4 text-[12px] italic leading-relaxed text-muted-foreground">
        {tx(lang, "every pattern on Insights shows \"why am I seeing this?\" — and any pattern can be set aside there.")}
      </p>
    </TactileCard>
  );
}
...
function LanguageSection() {
  ...
  return (
    <TactileCard>
      ...
      <div className="mt-4 flex gap-2" role="group" aria-label={tx(lang, "Display language")}>
      ...
      <p className="mt-3 text-[12px] italic leading-relaxed text-muted-foreground">
        {tx(lang, "InnerMate already answers in the language you write to it — हिन्दी, Hinglish, or English.")}
      </p>
    </TactileCard>
  );
}
...
function LatchSection() {
  ...
  const arm = async () => {
    ...
    try {
      ...
      toast.success(tx(lang, "The latch is on for this device."));
    } catch {
      toast.error(tx(lang, "Could not set the latch on this device."));
  ...
  const disarm = () => {
    try {
      ...
      toast.success(tx(lang, "The latch is off."));
    } catch {
      toast.error(tx(lang, "Could not take the latch off — try once more."));
    }
  };
  ...
  return (
    <TactileCard>
      <h2 className="font-serif text-xl">{tx(lang, "The latch")}</h2>
      <p className="mt-1.5 text-sm leading-relaxed text-muted-foreground">
        {tx(lang, "A four-digit key for this device only. It keeps glances out when a phone is held or borrowed — it is not encryption, and your account keeps protecting your words either way. Pick four digits you don't use anywhere else — not your phone or UPI PIN. If the app stays hidden for more than about a minute and a half it asks for the key again; quicker switches back don't. The steady room is never latched. Forget the key? Signing out and back in removes it.")}
      </p>
      {!armed && !editing && (
        <div className="mt-4">
      ...
      {editing && (
        <div className="mt-4 space-y-3">
          <div className="flex flex-wrap gap-4">
            {pinInput("latch-new", pin1, setPin1, tx(lang, "Four digits"))}
            {pinInput("latch-again", pin2, setPin2, tx(lang, "Once more"))}
          </div>
          {pin2.length === 4 && pin1 !== pin2 && (
            <p className="text-[12.5px] italic text-muted-foreground">{tx(lang, "Those two don't match yet.")}</p>
          )}
          <div className="flex gap-2">
            <Button variant="outline" className="min-h-11 rounded-full" disabled={pin1.length !== 4 || pin1 !== pin2 || busy} onClick={() => void arm()}>
              {armed ? tx(lang, "Change the key") : tx(lang, "Latch this device")}
            </Button>
      ...
      {armed && !editing && !removing && (
        <div className="mt-4 flex flex-wrap gap-2">
          <Button variant="outline" className="min-h-11 rounded-full" onClick={() => setEditing(true)}>{tx(lang, "Change the key")}</Button>
          <Button variant="ghost" className="min-h-11 rounded-full" onClick={() => setRemoving(true)}>{tx(lang, "Take the latch off")}</Button>
        </div>
      )}
      {removing && (
        <div className="mt-4 space-y-3">
          <p className="text-[13px] leading-relaxed text-muted-foreground">
            {tx(lang, "The latch comes off this device — no key needed here, since being signed in already proves it's you. Your account keeps protecting everything.")}
          </p>
          <div className="flex gap-2">
...
function SignOutCard() {
  ...
  return (
    <TactileCard>
      <h2 className="font-serif text-xl">{tx(lang, "Leaving for now")}</h2>
      <p className="mt-1.5 text-sm leading-relaxed text-muted-foreground">
        {tx(lang, "Your sanctuary stays exactly as you left it.")}
      </p>
      <div className="mt-4">
        <Button variant="outline" className="rounded-full" onClick={async () => { await supabase.auth.signOut(); navigate({ to: "/" }); }}>{tx(lang, "Sign out")}</Button>
      </div>
    </TactileCard>
  );
}
...
function SafetySection() {
  ...
  return (
    <TactileCard>
      <h2 className="font-serif text-xl">{tx(lang, "What this space is, and isn't")}</h2>
      <p className="mt-1.5 text-sm leading-relaxed text-muted-foreground">
        {tx(lang, "My Quiet Space is for self-reflection and emotional wellness. It is not therapy, medical advice, or emergency support. If you feel at risk, contact emergency services or a crisis helpline — India: ")}{helplines}.
      </p>
    </TactileCard>
  );
}
...
function VaultSection() {
  ...
  const [tagline, setTagline] = useState(tx(lang, "Private — for me alone."));
  ...
  const download = async () => {
    ...
    try {
      ...
      toast.success(tx(lang, "Your data has been exported."));
    } catch (e) {
      toast.error(e instanceof Error ? e.message : tx(lang, "Could not export your data."));
    } finally {
      setExporting(false);
    }
  };
  ...
  const wipe = async () => {
    ...
    try {
      ...
      toast.success(tx(lang, "Your data has been quietly cleared."));
      navigate({ to: "/home" });
    } catch (e) {
      toast.error(e instanceof Error ? e.message : tx(lang, "Could not delete your data."));
  ...
  const downloadPdf = async () => {
    ...
    try {
      ...
      a.click();
      URL.revokeObjectURL(url);
      toast.success(tx(lang, "Your private archive is ready."));
    } catch (e) {
      toast.error(e instanceof Error ? e.message : tx(lang, "Could not build PDF."));
    } finally {
      setPdfLoading(false);
    }
  };
  ...
  return (
    <TactileCard>
      <h2 className="font-serif text-xl">{tx(lang, "Everything here is yours")}</h2>
      <p className="mt-1.5 text-sm leading-relaxed text-muted-foreground">
        {tx(lang, "Take it with you, or let it all go.")}
      </p>
      <div className="mt-4 flex flex-wrap gap-2">
          ...
          <div className="rounded-xl border border-border/40 bg-muted/20 p-3 space-y-2.5">
            <label className="flex items-start gap-2.5 cursor-pointer">
              ...
              <div className="flex-1 min-w-0">
                <span className="text-sm">{tx(lang, "Include a privacy tagline on the cover")}</span>
                {includeTagline && (
                  <Input
                    value={tagline}
                    onChange={(e) => setTagline(e.target.value)}
                    placeholder={tx(lang, "Private — for me alone.")}
            ...
            <label className="flex items-center gap-2.5 cursor-pointer">
              ...
              <span className="text-sm">{tx(lang, "Include the date range of my entries")}</span>
            </label>
            <label className="flex items-center gap-2.5 cursor-pointer">
              <Checkbox checked={includeLastReflection} onCheckedChange={(v) => setIncludeLastReflection(v === true)} />
              <span className="text-sm">{tx(lang, "Include my last check-in reflection as a cover note")}</span>
            </label>
            <label className="flex items-center gap-2.5 cursor-pointer">
              <Checkbox checked={includeLastMoodCheckin} onCheckedChange={(v) => setIncludeLastMoodCheckin(v === true)} />
              <span className="text-sm">{tx(lang, "Include my last mood check-in (mood + tags) on the cover")}</span>
            </label>
          </div>
          <Button variant="outline" className="rounded-full" onClick={downloadPdf} disabled={pdfLoading}>
            {pdfLoading ? tx(lang, "Preparing PDF…") : tx(lang, "Download letters & check-ins (PDF)")}
          </Button>
          <p className="mt-2 text-xs italic leading-relaxed text-muted-foreground">
            {tx(lang, "Hindi in this PDF is legible but not perfectly typeset — a matra may sit slightly off.")}
          </p>
        </div>
        <Button variant="outline" className="rounded-full" onClick={download} disabled={exporting}>
          {exporting ? tx(lang, "Exporting…") : tx(lang, "Export my data (JSON)")}
        </Button>
        <Button
        ...
        >
          {tx(lang, "a letter for someone you trust →")}
        </Link>
        <p className="w-full text-xs text-muted-foreground italic">
          {tx(lang, "Delete removes everything you created — journals, moods, AI chats, memories, letters, reflections, paths. A minimal audit trail (consent, rights requests, safety events) is retained as legally required.")}
        </p>
      </div>
      ...
      {confirmingDelete && (
        <div
          role="alertdialog"
          aria-label={tx(lang, "Confirm deleting your data")}
        ...
        >
          <p className="font-serif text-[13px] italic" style={{ color: "color-mix(in oklab, var(--ink) 66%, var(--paper))" }}>
            {tx(lang, "before this book closes")}
          </p>
          <p className="mt-2 font-serif text-[19px] font-light leading-snug">
            {tx(lang, "Delete all your journals, moods, conversations, memories, letters, and reflections?")}
          </p>
          <p id="delete-confirm-detail" className="mt-2.5 text-[13.5px] leading-relaxed" style={{ color: "color-mix(in oklab, var(--ink) 80%, var(--paper))" }}>
            {tx(lang, "Only legally required consent/safety records will remain. This cannot be undone — it is a true goodbye to everything written here.")}
          </p>
          <div className="mt-4 flex flex-wrap items-center gap-6 border-t pt-4" style={{ borderColor: "color-mix(in oklab, var(--ink) 14%, transparent)" }}>
            ...
            >
              {deleting ? tx(lang, "clearing…") : tx(lang, "yes — clear everything")}
            </button>
          </div>
        </div>
      )}
...
function PasswordSection() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    if (password.length < 6) {
      toast.error(tx(lang, "Password must be at least 6 characters."));
      return;
    }
    setLoading(true);
    try {
      ...
      toast.success(tx(lang, "Password updated quietly."));
      setPassword("");
      setConfirm("");
    } catch (err) {
  ...
  return (
    <TactileCard>
      <h2 className="font-serif text-xl">{tx(lang, "A new password")}</h2>
      <p className="mt-1.5 text-sm leading-relaxed text-muted-foreground">{tx(lang, "Change the key to your sanctuary whenever you like.")}</p>
      <form onSubmit={submit} className="mt-4 w-full space-y-4">
        ...
        <Button type="submit" disabled={loading} variant="outline" className="rounded-full">
          {loading ? tx(lang, "Saving…") : tx(lang, "Update password")}
        </Button>
      </form>
    </TactileCard>
  );
...
function ToneSection() {
  ...
  const select = async (tone: "gentle" | "poetic" | "practical" | null) => {
    ...
    try {
      ...
      toast.success(tone ? `${tx(lang, TONE_OPTIONS.find(t => t.value === tone)?.label ?? "")} ${tx(lang, "tone saved.")}` : tx(lang, "Default tone reset."));
    } catch (e) {
  ...
  return (
    <TactileCard>
      <h2 className="font-serif text-xl">{tx(lang, "How InnerMate speaks with you")}</h2>
      <p className="mt-1.5 text-sm leading-relaxed text-muted-foreground">
        {tx(lang, "Choose the voice that feels right for your next conversation. You can change it anytime, even mid-chat.")}
      </p>
          ...
          return (
            ...
            >
              <div className="flex items-center justify-between">
                <span className={`text-sm font-medium ${active ? "text-foreground" : "text-muted-foreground"}`}>{tx(lang, t.label)}</span>
                {active && <span className="h-2 w-2 rounded-full bg-primary" />}
              </div>
              <p className="mt-1 text-xs leading-relaxed text-muted-foreground">{tx(lang, t.description)}</p>
            </button>
          );
        })}
      </div>
      {currentTone && (
        <div className="mt-4 rounded-xl border border-border/40 bg-muted/30 px-4 py-3">
          <p className="text-[11px] uppercase tracking-wider text-muted-foreground/70">{tx(lang, "How it might sound")}</p>
          <p className="mt-1.5 font-serif text-sm italic leading-relaxed text-foreground/90">
            “{tx(lang, TONE_OPTIONS.find(t => t.value === currentTone)?.preview ?? "")}”
          </p>
        </div>
      )}
    </TactileCard>
...
function BackgroundAnimationSection() {
  ...
  const toggle = async () => {
    ...
    try {
      ...
      await qc.invalidateQueries({ queryKey: ["profile"] });
      toast.success(!enabled ? tx(lang, "The sky is drifting again.") : tx(lang, "The sky is holding still."));
    } catch (e) {
      toast.error((e as Error).message);
    } finally {
      setSaving(false);
  ...
  return (
    <TactileCard>
      <div className="flex items-start justify-between gap-4">
        <div className="min-w-0">
          <h2 className="font-serif text-xl">{tx(lang, "The slow drift of the sky")}</h2>
          <p className="mt-1.5 text-sm leading-relaxed text-muted-foreground">
            {tx(lang, "A gentle shifting glow behind everything here. Turn it off for a completely still sky — kinder for comfort, focus, or motion sensitivity.")}
          </p>
        </div>
        <button
          role="switch"
          aria-checked={enabled}
          aria-label={tx(lang, "Toggle background animation")}
          onClick={toggle}
          disabled={saving}
      ...
      <p className="mt-3 text-xs text-muted-foreground/80">
        {tx(lang, "Reduced-motion system settings are always respected.")}
      </p>
    </TactileCard>
  );
}
...
function InnerMapSection() {
  ...
  const toggle = async () => {
    try {
      ...
      toast.success(!readable ? tx(lang, "InnerMate may read your map.") : tx(lang, "The map is quiet now."));
    } catch (e) { toast.error((e as Error).message); }
  };
  ...
  if (story?.is_ai_readable) {
    if (story.roots) previewLines.push(`${tx(lang, "Roots: ")}${story.roots}`);
    if (story.current_chapter) previewLines.push(`${tx(lang, "Current chapter: ")}${story.current_chapter}`);
    if (story.people) previewLines.push(`${tx(lang, "People who matter: ")}${story.people}`);
    if (story.healing_from) previewLines.push(`${tx(lang, "Healing from: ")}${story.healing_from}`);
    if (story.speaking_preference) previewLines.push(`${tx(lang, "How they like to be spoken to: ")}${story.speaking_preference}`);
  }

  return (
    <TactileCard>
      <h2 className="font-serif text-xl">{tx(lang, "Landmarks you have chosen to share")}</h2>
      <p className="mt-1.5 text-sm leading-relaxed text-muted-foreground">
        {tx(lang, "This is the inner map — the parts of your story you have placed here yourself, so InnerMate can meet you where you actually are. Every landmark is optional. Share only what feels right.")}
      </p>

      <div className="mt-5 flex items-start justify-between gap-4 rounded-xl border border-border/50 bg-muted/20 p-4">
        <div className="min-w-0">
          <p className="text-sm font-medium">{tx(lang, "InnerMate may read this")}</p>
          <p className="mt-1 text-xs leading-relaxed text-muted-foreground">
            {tx(lang, "When off, every landmark stays saved here — quiet, and never sent to InnerMate.")}
          </p>
        </div>
        <button
          role="switch"
          aria-checked={readable}
          aria-label={tx(lang, "Let InnerMate read your inner map")}
          onClick={toggle}
      ...
      <div className="mt-7 rounded-xl border border-border/40 bg-muted/30 p-4">
        <p className="text-[11px] uppercase tracking-wider text-muted-foreground/80">{tx(lang, "What InnerMate can currently see")}</p>
        <p className="mt-2 whitespace-pre-wrap text-sm italic leading-relaxed text-foreground/85">
          {previewLines.length ? previewLines.join("\n") : tx(lang, "Nothing — the map is resting.")}
        </p>
      </div>

      <p className="mt-5 text-xs leading-relaxed text-muted-foreground/90">
        {tx(lang, "You can view, edit, quiet, or erase any of this whenever you like. InnerMate only ever reads what you allow.")}
      </p>
    </TactileCard>
  );
}
...
function StoryField({ field, label, hint, initial }: { field: StoryKey; label: string; hint: string; initial: string }) {
  ...
  return (
    <div>
      <Label className="font-serif text-base">{tx(lang, label)}</Label>
      <p className="mt-0.5 text-xs text-muted-foreground/90">{tx(lang, hint)}</p>
      <Textarea
        value={value}
      ...
      <div className="mt-2 flex items-center justify-between">
        ...
        <div className="flex gap-2">
          {initial && (
            <Button variant="outline" size="sm" className="rounded-full" onClick={erase} disabled={saving}>{tx(lang, "Erase")}</Button>
          )}
          <Button size="sm" className="rounded-full" onClick={save} disabled={!dirty || saving}>
            {saving ? tx(lang, "Keeping…") : tx(lang, "Keep this")}
          </Button>
        </div>
      </div>
    </div>
...
function SundayLetterSection() {
  ...
  return (
    <TactileCard>
      <h2 className="font-serif text-xl">{tx(lang, "A letter from your week, every Sunday")}</h2>
      <p className="mt-1.5 text-sm leading-relaxed text-muted-foreground">
        {tx(lang, "A short, private letter from InnerMate, waiting for you each Sunday. It notices your week with care, and ends with one small ritual for the days ahead. Nothing is sent or shared.")}
      </p>

      <div className="mt-5 flex items-start justify-between gap-4 rounded-xl border border-border/50 bg-muted/20 p-4">
        <div className="min-w-0">
          <p className="text-sm font-medium">{tx(lang, "Receive a weekly letter")}</p>
          <p className="mt-1 text-xs leading-relaxed text-muted-foreground">
            {tx(lang, "When on, an envelope quietly appears on Home each Sunday.")}
          </p>
        </div>
        <button
          role="switch" aria-checked={enabled}
          aria-label={tx(lang, "Receive a weekly letter")}
      ...
      {enabled && (
        <div className="mt-3 flex items-start justify-between gap-4 rounded-xl border border-border/50 bg-muted/20 p-4">
          <div className="min-w-0">
            <p className="text-sm font-medium">{tx(lang, "May the letter walk among your stars?")}</p>
            <p className="mt-1 text-xs leading-relaxed text-muted-foreground">
              {tx(lang, "If yes, the letter may gently reference one memory or detail you've already marked shareable. Never more than one.")}
            </p>
          </div>
          <button
            role="switch" aria-checked={usesMemories}
            aria-label={tx(lang, "Let the weekly letter reference one shareable memory")}
