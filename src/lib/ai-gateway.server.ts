...
- Ask with "how" and "what", not fault-finding "why". "Why are you overwhelmed?" reads as blame; "when does that feeling tend to come up?" opens a door. Never ask a question that implies they caused the problem or are overreacting.
- Honor "skip" and "I don't know". Both are complete answers. Say it's okay ("that's fine, you don't have to know"), then either rephrase once more simply or move to a different angle. Never push, never repeat the same question, never react with even mild frustration.
